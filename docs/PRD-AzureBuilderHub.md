...
| Database | Azure Cosmos DB (NoSQL) | Flexible schema for project metadata, global distribution if needed |
...
| Search | Cosmos DB full-text (Phase 1) → Azure AI Search (Phase 4) | Start simple, upgrade when query complexity demands it |
...
### 4.2 Hub Database Schema (Cosmos DB)

**Projects Collection**

| Field | Type | Description |
|-------|------|-------------|
| `id` | string | Unique project ID (generated by hub) |
| `name` | string | Display name |
| `description` | string | Long description |
| `authorId` | string | Entra ID user identifier |
| `authorName` | string | Display name |
| `status` | enum | `draft` \| `published` \| `archived` |
| `tags` | string[] | Searchable tags |
| `azureServices` | string[] | Azure services referenced |
| `layout` | string | `full-width` \| `side-panel` |
| `pageCount` | number | Number of pages in the project |
| `currentVersion` | number | Latest version number |
| `starCount` | number | Community stars |
| `forkCount` | number | Times forked |
| `forkedFrom` | object \| null | `{ projectId, projectName, authorName }` — attribution chain |
| `thumbnailUrl` | string | Blob Storage URL for thumbnail |
| `previewUrl` | string | Blob Storage URL for built Vite app (live preview) |
| `createdAt` | datetime | First upload timestamp |
| `updatedAt` | datetime | Last upload timestamp |
| `publishedAt` | datetime \| null | When published to community |

**Versions Collection**

| Field | Type | Description |
|-------|------|-------------|
| `id` | string | `{projectId}-v{version}` |
| `projectId` | string | Parent project |
| `version` | number | Sequential version number |
| `bundleUrl` | string | Blob Storage URL for the version's file bundle |
| `manifest` | object | Snapshot of `project.json` at this version |
| `createdAt` | datetime | Upload timestamp |

**Stars Collection** (lightweight — could also be a sub-document)

| Field | Type | Description |
|-------|------|-------------|
| `userId` | string | Who starred |
| `projectId` | string | What was starred |
...
5. Hub creates a draft project entry in Cosmos DB
...
- [ ] Scaffold AzureBuilderHub repo (SWA + Functions + Cosmos DB)
